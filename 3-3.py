...
# 定義非圓形分布函數 (例如，橢圓形分布)
def elliptical_distribution(x1, x2):
    return np.exp(-0.1 * ((x1/1.5)**2 + x2**2))  # Adjusted for an elliptical shape

# 生成1000筆非圓形分布的 x1, x2 資料
np.random.seed(0)
x1 = np.random.normal(0, np.sqrt(15), 1000)  # Wider spread in x1 for elliptical shape
x2 = np.random.normal(0, np.sqrt(5), 1000)   # Narrower spread in x2
z = elliptical_distribution(x1, x2)
data = np.column_stack((x1, x2, z))

# Streamlit app
st.title("3D Non-Circular Data Classification with SVM")
